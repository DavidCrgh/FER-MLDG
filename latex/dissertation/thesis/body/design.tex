...
\textbf{Pretrained models:} \label{section:design-pretrained-models} The proposed approach requires access to model weights for the APViT network pretrained of the AffectNet dataset. To the best of our knowledge, these weights are not publicly available. As such, we retrained the network from scratch using the same procedure as described in the original APViT paper \cite{apvit2023}.

\textbf{Computational resources:} The main computational resource required for executing the proposed approach is the available GPU memory. Phase 1 of the design requires more memory since it leverages the two heaviest networks: APViT and the modified LDG. In executions of phase 1, we noticed a memory requirement of aproximately 16GB of VRAM per training run.
...
\textbf{Software environment:} Execution of the training scripts requires a environment with the following software:
\begin{itemize}
    \item Python 3.10
...
The main code repository is hosted on GitHub and can be accessed at the following URL: \url{https://github.com/to-do-add-repo-link}. The main branch holds the training scripts for both phases of the proposed approach. The \texttt{networks} directory contains the implementations of the APViT, LDG, and EfficientFace architectures. Dataset preprocessing code, including face alignment with RetinaFace and conversion to the formats required by each network, is located in the \texttt{preproc} directory.

Experiment execution is organized through Jupyter notebooks. Phase I experiments are run from notebooks in \texttt{notebooks/phase1}, with each notebook corresponding to one combination of the ablation factors (pretrained weights, local-global modules, transformer guidance, and dataset variant). Phase II notebooks in \texttt{notebooks/phase2} automate the training of EfficientFace with different label generators. The notebooks handle environment setup, configuration loading, and invocation of the underlying training scripts. Notebooks are also provided for executing the hyperparameter optimization process using WandB Sweeps.