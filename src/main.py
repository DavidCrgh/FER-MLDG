# main.py
# Train EfficientFace on AffectNet with parameterized label distribution generation using APViT or mLDG
import argparse
import os
...
from networks.EfficientFace.models import EfficientFace
...
# New arguments for APViT and mLDG configurations
parser.add_argument('--label_generator', type=str, default='MLDG', choices=['APVIT', 'MLDG'], help='which label distribution generator to use')
...
parser.add_argument('--mldg_weights', type=str, default='./weights/mLDG_7class_best.pth', help='path to mLDG weights')

parser.add_argument('--image_size', type=int, default=112, help='input image size for models (square images only)')
...
parser.add_argument('--save_checkpoints', action='store_true', default=True, help='save checkpoints to disk')
parser.add_argument('--no_save_checkpoints', dest='save_checkpoints', action='store_false', help='disable saving checkpoints to disk')

parser.add_argument('--ef_weights', type=str, default='./weights/Pretrained_EfficientFace.tar', help='path to EfficientFace weights')

parser.add_argument('--use_upsampling', action='store_true', default=True, help='Upsample images computed by EfficientFace (requires: image_size >= 224)')
...
def main():
    ...
    print(f'Using label generator: {args.label_generator}')
    ...
    if not args.no_wandb:
        ...
        wandb.init(
            ...
            config={
                ...
                "label_generator": args.label_generator,
    ...
    # Create model
    ## mLDG (trainee model)
    model_cla = init_efficientface_model(args.ef_weights, num_classes=args.num_classes)
    
    ## APViT (label distribution generator) - only initialize if using it
    model_dis = None
    
    if args.label_generator == 'APVIT':
        # Small hack to ensure APViT outputs torch tensors
        os.environ['MODEL_VIS'] = '1'

        model_dis = init_apvit_model()

    elif args.label_generator == 'MLDG':
        model_dis = load_base_mLDG(             # TODO: check if this function will work with a full pretrained mLDG model
    ...
    with open(txt_name, 'a') as f:
        f.write(f'Using label generator: {args.label_generator}\n')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.no_wandb and args.save_checkpoints:
...
def train(train_loader, model_cla, model_dis, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # Compute output from EfficientFace
        # Upsample images if specified
        if args.use_upsampling and args.image_size < 224:
            output = model_cla(F.interpolate(images, size=(224, 224), mode='bilinear', align_corners=False))
        else:
...
def validate(val_loader, model, criterion, args, epoch=0):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            # Compute output from EfficientFace
            if args.use_upsampling and args.image_size < 224:
                output = model(F.interpolate(images, size=(224, 224), mode='bilinear', align_corners=False))
            else:
                output = model(images)
...
def init_efficientface_model(checkpoint_path : str, num_classes : int) -> nn.Module:
    ...
    model_cla.module.fc = nn.Linear(1024, num_classes).cuda()
...
def format_run_string(args):
    ...
    return f"{combination_index}_{args.label_generator}_NC{args.num_classes}_US_{args.use_upsampling}"
...
def rename_directories(dir_path):
    ...
    """
    warnings.warn(f"Renaming class directories at {dir_path}. Ensure this is intended.", UserWarning)
