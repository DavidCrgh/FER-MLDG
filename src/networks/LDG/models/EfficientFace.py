...
class LocalFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        # Divide the input maps spatially into 4 equal quadrants or patches
        _, _, h, w = x.shape
        h_half, w_half = h // 2, w // 2

        patch_11 = x[:, :, :h_half, :w_half]
        patch_21 = x[:, :, h_half:, :w_half]
        patch_12 = x[:, :, :h_half, w_half:]
        patch_22 = x[:, :, h_half:, w_half:]
